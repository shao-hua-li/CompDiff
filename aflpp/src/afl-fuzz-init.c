...
static void handle_existing_out_dir(afl_state_t *afl) {
  ...
  fn = alloc_printf("%s/diffs", afl->out_dir);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  tmp = alloc_printf("%s/diffs", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);
  ...
  if (!afl->in_place_resume) {
    ...
    fprintf(
        ...
        "unique_hangs, max_depth, execs_per_sec, total_execs, edges_found, unique_diffs\n");
...
void check_binary(afl_state_t *afl, u8 *fname, u8 is_cmplog) {
  ...
  if (strchr(fname, '/') || !(env_path = getenv("PATH"))) {
    ...
    if (!is_cmplog) {
      u32 f_len_diff = 0;
      u8* laf_loc = strstr(fname, "-laf");
      if (laf_loc) {
        laf_loc = '\0';
      }
      for (u32 i = 0; i < afl->diff_num; i++) {
        afl->diff_fsrv[i].target_path = alloc_printf("%s-%d", fname, i);
        if (stat(afl->diff_fsrv[i].target_path, &st) || !S_ISREG(st.st_mode) ||
            !(st.st_mode & 0111) || (f_len_diff = st.st_size) < 4) {
          FATAL("Program '%s-%d' not found or not executable", fname, i);
        }
      }
    }
