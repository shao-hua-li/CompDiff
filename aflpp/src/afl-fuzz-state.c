...
void afl_state_init(afl_state_t *afl, uint32_t map_size) {
  ...
  afl->virgin_diff = ck_alloc(map_size);
  ...
  for (u32 i = 0; i < afl->diff_num; i++) {
    afl->diff_fsrv[i].use_stdin = 1;
    afl->diff_fsrv[i].map_size = map_size;
    // afl_state_t is not available in forkserver.c
    afl->diff_fsrv[i].afl_ptr = (void *)afl;
    afl->diff_fsrv[i].add_extra_func = (void (*)(void *, u8 *, u32)) & add_extra;
    afl->diff_fsrv[i].exec_tmout = EXEC_TIMEOUT;
    afl->diff_fsrv[i].mem_limit = MEM_LIMIT;
    afl->diff_fsrv[i].dev_urandom_fd = -1;
