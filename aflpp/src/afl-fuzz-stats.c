...
void maybe_update_plot_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  fprintf(afl->fsrv.plot_file,
          ...
          afl->plot_prev_ed, t_bytes, afl->unique_diffs);                     /* ignore errors */
...
*/
  sprintf(tmp, "%s (%s%s unique)", u_stringify_int(IB(0), afl->total_diffs),
          u_stringify_int(IB(1), afl->unique_diffs),
          (afl->unique_diffs >= KEEP_UNIQUE_CRASH) ? "+" : "");

  SAYF(bSTG bV bSTOP "  total  diffs : " cRST "%-20s" bSTG bV "\n", tmp);
