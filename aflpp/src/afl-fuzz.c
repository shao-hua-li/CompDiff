...
int main(int argc, char **argv_orig, char **envp) {
  ...
  for (u32 i = 0; i < afl->diff_num; i++) { afl_fsrv_init(&(afl->diff_fsrv[i])); }
  ...
  while ((opt = getopt(
              argc, argv,
              "+b:y:Y:B:c:CdDe:E:hi:I:f:F:l:L:m:M:nNOo:p:RQs:S:t:T:UV:Wx:Z")) > 0) {

    switch (opt) {

      case 'y':
        if (sscanf(optarg, "%u", &(afl->diff_num)) < 1 ||
              optarg[0] == '-') FATAL("Invalid number of compilers: Bad syntax used for -y");
        break;
      case 'Y':                                    /* */
	      afl->output_file = optarg;
        afl->flag_use_output = 1;
        break;
  ...
  for (u32 i = 0; i < afl->diff_num; i++) {afl->diff_fsrv[i].kill_signal = afl->fsrv.kill_signal;}
  ...
  for (u32 i = 0; i < afl->diff_num; i++) {
    afl->diff_fsrv[i].out_file = afl->fsrv.out_file;
    afl->diff_fsrv[i].out_dir_fd = afl->fsrv.out_dir_fd;
    afl->diff_fsrv[i].out_fd = afl->fsrv.out_fd;
  }
  setup_stdout_file_diff(afl);
  ...
  if (afl->cmplog_binary) {
    ...
    if (!afl->fsrv.qemu_mode && !afl->fsrv.frida_mode &&
      ...
      check_binary(afl, afl->cmplog_binary, 1);

    }

  }

  check_binary(afl, argv[optind], 0);
  ...
  if (afl->shmem_testcase_mode) { 
    ...
    for (u32 i = 0; i < afl->diff_num; i++) {
      afl->diff_fsrv[i].support_shmem_fuzz = afl->fsrv.support_shmem_fuzz;
      afl->diff_fsrv[i].shmem_fuzz_len = afl->fsrv.shmem_fuzz_len;
      afl->diff_fsrv[i].shmem_fuzz = afl->fsrv.shmem_fuzz;
    }
  }
  ...
  if (afl->non_instrumented_mode || afl->fsrv.qemu_mode ||
    ...
    afl->virgin_diff = ck_realloc(afl->virgin_diff, map_size);
  ...
  for (u32 i = 0; i < afl->diff_num; i++) {
    afl->diff_fsrv[i].trace_bits =
      afl_shm_init(&afl->shm, afl->diff_fsrv[i].map_size, afl->non_instrumented_mode);
  }
  ...
  if (!afl->non_instrumented_mode && !afl->fsrv.qemu_mode &&
    ...
    for (u32 i = 0; i < afl->diff_num; i++) {
      afl_fsrv_start_diff(&afl->diff_fsrv[i], i, afl->argv, &afl->stop_soon, afl->afl_env.afl_debug_child);
    }
    ...
    if (map_size < new_map_size) {
      ...
      afl->virgin_diff = ck_realloc(afl->virgin_diff, new_map_size);
  ...
  memset(afl->virgin_diff, 255, map_size);
